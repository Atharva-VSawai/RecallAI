...
@router.post("/upload")
async def upload(file: UploadFile = File(...), user: AuthenticatedUser = Depends(get_current_user), project: ProjectContext = Depends(require_project_permission("knowledge:write")), x_llm_provider: str = Header(default="groq")):
...
@router.post("/slack")
async def slack(request: SlackIngestRequest, user: AuthenticatedUser = Depends(get_current_user), project: ProjectContext = Depends(require_project_permission("knowledge:write")), x_llm_provider: str = Header(default="groq")):
...
async def audio(file: UploadFile = File(...), user: AuthenticatedUser = Depends(get_current_user), project: ProjectContext = Depends(require_project_permission("knowledge:write")), x_llm_provider: str = Header(default="groq")):
    ...
    return await upload(file, user, project, x_llm_provider)
