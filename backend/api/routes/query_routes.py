...
@router.post("")
def query(request: QueryRequest, user: AuthenticatedUser = Depends(get_current_user), project: ProjectContext = Depends(require_project_permission("knowledge:read")), x_llm_provider: str = Header(default="groq")):
