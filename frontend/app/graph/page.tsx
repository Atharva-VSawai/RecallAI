...
function sourceLabel(source: string, displayName?: string) {
  if (displayName) return displayName;
  const [kind, ...rest] = source.split(":");
  return rest.length ? rest.join(":") : kind;
}

export default function GraphPage() {
  const { can } = useAuth();
  const { resolvedTheme } = useTheme();
  const graphSurface = resolvedTheme === "dark" ? "#0f1719" : "#f7f8f6";
  const graphText = resolvedTheme === "dark" ? "#eef6f3" : "#17231f";
  ...
  const sourceNames = useMemo(() => {
    ...
    data.nodes.forEach((node) => { if (node.source && node.source_label) names.set(node.source, node.source_label); });
    return names;
  }, [data.nodes]);
  ...
  const nodeCanvasObject = useCallback((rawNode: ForceNode, context: CanvasRenderingContext2D, scale: number) => {
    ...
    context.fill();
    context.strokeStyle = isSelected ? graphText : (resolvedTheme === "dark" ? "rgba(238,246,243,.55)" : "rgba(23,35,31,.35)");
    ...
    if (node.type === "Decision" || isSelected || highlightedIds.has(node.id)) {
      ...
      context.font = `${11 / scale}px Inter, sans-serif`; context.fillStyle = isDimmed ? (resolvedTheme === "dark" ? "rgba(130,154,149,.35)" : "rgba(95,111,104,.35)") : graphText;
      context.textAlign = "center"; context.textBaseline = "top"; context.fillText(label, x, y + radius + 4 / scale);
    }
  }, [graphText, highlightedIds, resolvedTheme, selected?.id]);
  ...
  const linkColor = useCallback((rawLink: ForceLink) => {
    ...
    return connected ? (resolvedTheme === "dark" ? "rgba(178,196,191,.45)" : "rgba(95,111,104,.42)") : (resolvedTheme === "dark" ? "rgba(130,154,149,.10)" : "rgba(95,111,104,.10)");
  }, [highlightedIds, resolvedTheme]);
  ...
  return <div data-can-upload={can("knowledge:write")} className="flex h-[calc(100dvh-var(--topbar-height))] min-h-[540px] flex-col overflow-hidden bg-background">
