...
def test_request_embedding_cache_reuses_same_query_vector():
    ...
    with patch.object(chroma, "_embeddings", fake_embeddings), patch.object(chroma, "_collection", fake_collection):
        with chroma.query_embedding_cache():
            chroma.chroma_search("migration", project_id="project-a", organization_id="org-a")
            chroma.chroma_search("migration", project_id="project-a", organization_id="org-a")
...
def test_unauthorized_project_is_rejected_before_data_queries(monkeypatch):
    ...
    with patch("application.services.project_service._driver", driver):
        session.run.side_effect = [[], SimpleNamespace(single=lambda: None), [], []]
...
def test_new_user_project_context_bootstraps_default_workspace():
    class Result:
    ...
    session = MagicMock()
    session.run.side_effect = [
        Result(),  # initial list_projects: no memberships
        Result(record=project_record),  # ensure_default_project
        Result(),  # membership de-duplication
        Result(records=[project_record]),  # list_projects after bootstrap
    ]
    driver = MagicMock()
    driver.session.return_value.__enter__.return_value = session
    user = AuthenticatedUser("user-new", "org-new", "USER", "new@example.test")
    with patch("application.services.project_service._driver", driver):
        context = ProjectService().get_project_context(user, None)
    assert context.project_id == "main-abc123"
    assert context.organization_id == "org-new"
...
def test_invalid_jwt_does_not_expose_decode_details():
    service = __import__("application.services.auth_service", fromlist=["AuthService"]).AuthService()
    with patch("application.services.auth_service.settings.supabase_url", "https://project.supabase.co"), \
         ...
         patch.object(service, "_decode_with_jwks", side_effect=__import__("jwt").InvalidSignatureError("secret details")):
        with pytest.raises(Exception) as exc_info:
            service.authenticate("bad-token")
    assert "secret details" not in str(exc_info.value)


def test_patch_is_allowed_by_cors():
    from fastapi.testclient import TestClient
    from main import app
    response = TestClient(app).options(
        ...
        headers={
            ...
            "Access-Control-Request-Headers": "authorization,content-type,x-project-id",
        },
    )
...
def test_same_named_legacy_source_is_rejected_when_owners_are_ambiguous():
    from db import chroma
    class Result:
        def single(self):
            return {"owners": ["org-a:project-a", "org-b:project-b"]}
    session = MagicMock()
    session.run.return_value = Result()
    driver = MagicMock()
    driver.session.return_value.__enter__.return_value = session
    with patch("db.neo.get_driver", return_value=driver):
        assert chroma._source_authorized_in_neo4j("org-a", "project-a", "document:report.pdf") is False


def test_canonical_document_ids_are_content_addressed():
    from application.services.ingestion_service import canonical_document_id
    assert canonical_document_id("document", "a" * 64) == "document:" + "a" * 64
    assert canonical_document_id("document", "a" * 64) != canonical_document_id("document", "b" * 64)


def test_persisted_membership_overrides_untrusted_org_claim():
    from application.services.auth_service import _organization_id
    session = MagicMock()
    session.run.return_value.data.return_value = [{"organization_id": "user:user-1"}]
    driver = MagicMock()
    driver.session.return_value.__enter__.return_value = session
    with patch("db.neo.get_driver", return_value=driver):
        assert _organization_id({"app_metadata": {"organization_id": "other-org"}}, "user-1") == "user:user-1"
