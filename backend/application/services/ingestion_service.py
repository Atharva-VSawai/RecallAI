...
class IngestionService:
    ...
    def ingest_upload(self, content: bytes, filename: str, user: AuthenticatedUser, provider: str, project: "ProjectContext | None" = None) -> dict:
        extension = filename.rsplit(".", 1)[-1].lower() if "." in filename else ""
        source_type = SUPPORTED_EXTENSIONS.get(extension)
        if not source_type:
            raise ValidationError("Unsupported file type")
        project = project or _FallbackProjectContext()
        try:
            content_hash = _compute_hash(content)
            from db.file_registry import check_file_exists, register_file
            existing = check_file_exists(content_hash, project.project_id)
            if existing:
            ...
            result = run_ingestion(content, filename, source, provider=provider, project_id=project.project_id, organization_id=project.organization_id)
            register_file(filename, content_hash, extension, source, project.project_id, project.organization_id)
            from activity_store import activity_store
            activity_store.add_event("ingest", f"File ingested: {filename}", f"Processed {len(content)} bytes", source, user_id=user.user_id, project_id=project.project_id)
            return {"status": "success", "duplicate": False, "result": result, "source": source, "project_id": project.project_id}
    ...
    def ingest_slack(self, channel_id: str, limit: int, user: AuthenticatedUser, provider: str, project: "ProjectContext | None" = None) -> dict:
        project = project or _FallbackProjectContext()
        try:
            ...
            result = run_ingestion_from_text(raw_text, source, provider=provider, project_id=project.project_id, organization_id=project.organization_id)
            from activity_store import activity_store
            activity_store.add_event("slack", f"Slack ingestion: #{channel_id}", f"Processed {limit} messages from channel", source, user_id=user.user_id, project_id=project.project_id)
            return {"status": "success", "result": result, "source": source, "project_id": project.project_id}
        except Exception as exc:
