...
def _find_related_decisions(topic: str, source_filter: Optional[str] = None, project_id: Optional[str] = None, organization_id: Optional[str] = None) -> str:
    records = neo_impact_search(topic, source_filter=source_filter, project_id=project_id, organization_id=organization_id)
    if not records:
...
def _find_decisions_by_person(person_name: str, source_filter: Optional[str] = None, project_id: Optional[str] = None, organization_id: Optional[str] = None) -> str:
    records = _neo_search(person_name, source_filter=source_filter, project_id=project_id, organization_id=organization_id)
    if not records:
