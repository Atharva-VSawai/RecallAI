...
import { Bell, ChevronDown, CircleHelp, Database, FileSearch, FolderOpen, GitBranch, LayoutDashboard, LogOut, Menu, PanelLeftClose, PanelLeftOpen, Settings, ShieldCheck, UserCircle, Users, X } from "lucide-react";
...
export default function Navbar() {
  ...
  return <>
    <header className="fixed inset-x-0 top-0 z-50 h-14 border-b border-card-border bg-background-secondary">
    ...
    {mobileOpen && <button className="fixed inset-0 top-14 z-30 bg-black/45 md:hidden" aria-label="Close navigation" onClick={() => setMobileOpen(false)} />}
    <aside style={{ width: sidebarWidth }} className={`${mobileOpen ? "translate-x-0" : "-translate-x-full md:translate-x-0"} fixed bottom-0 left-0 top-14 z-40 border-r border-card-border bg-background-secondary transition-[width,transform] duration-150`}>
