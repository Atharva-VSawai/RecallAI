...
export default function SourcesPage() {
  ...
  const load = useCallback(async () => {
    ...
    try { setFiles(await listFiles()); } catch { setFiles([]); setError(true); } finally { setLoading(false); }
  }, [activeProjectId]);
  ...
  return <div className="page-container">
    <div className="page-header"><div><p className="page-eyebrow">Knowledge management</p><h1 className="page-title">Sources</h1><p className="page-description">Review and manage the files indexed in {activeProject?.name ?? "this workspace"}.</p></div><Button asChild><Link href="/query?tab=upload"><Upload size={16} />Add knowledge</Link></Button></div>
